...
SliderUnlock.prototype.handerMouseUp = function () { //先
    var _self = this;
    _self.opts.isRun=true;
    // 停止 stop
    _self.swipestart = false;
    // _self.move();
    if (_self.index < _self.max) {
    ...
    if (_self.index == _self.max && _self.max > 0 && _self.isOk) {
       _self.success();
       _self.opts.isRun=false;
    }
}

SliderUnlock.prototype.handerMouseOut = function () {
    
    var _self = this;
    if(_self.opts.isRun) return;
    // 停止 stop
    _self.swipestart = false;
    // _self.move();
 ...
 */
SliderUnlock.prototype.move = function (event) {
    var _self = this;
