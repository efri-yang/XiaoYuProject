...
SliderUnlock.prototype.init = function () {
    ...
    _self.updateView();
    _self.elm.find(".slideunlock-btn").on("mousedown", function (event) {
        ...
        var e = event || window.event;
        _self.lableIndex = e.targetTouches[0].pageX - this.offsetLeft;
        _self.handerIn();
...
SliderUnlock.prototype.handerMove = function (event, type) {
    ...
    if (_self.swipestart) {
        ...
        if (type == "mobile") {
            _self.index = event.targetTouches[0].pageX - _self.lableIndex;
        } else {
...
SliderUnlock.prototype.reset = function () {
    ...
    _self.index = 0;
    _self.elm.find(".slideunlock-btn").animate({
            left:_self.index,
        }, _self.opts.duration)
        .next(".slideunlock-txt").animate({
            opacity: 1
        }, _self.opts.duration);
...
SliderUnlock.prototype.updateView = function () {
    ...
    if (_self.index == (_self.max - 0)) {
        ...
        _self.elm.addClass("success").find(".slideunlock-txt").html(_self.opts.successLabelTip).css(style);
    } else {
        $(".slideunlock-lockable").val(0);
        _self.elm.removeClass("success").find(".slideunlock-txt").html(_self.opts.labelTip);
    }
    _self.always();
}
